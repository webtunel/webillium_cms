...
<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{isset($form['style']) ? $form['style'] : ''}}">
    <label class='control-label'>{{$form['label']}}
        ...
        <select style='width:100%' class='form-control select2 select2-hidden-accessible' id="{{$name}}" tyle="width: 100%;" tabindex="-1" aria-hidden="true"
                {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} name="{{$name}}{{(isset($form['relationship_table']) && $form['relationship_table'])?'[]':''}}" {{ (isset($form['relationship_table']) && $form['relationship_table'])?'multiple="multiple"':'' }} >
