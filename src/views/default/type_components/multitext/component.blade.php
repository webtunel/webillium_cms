    ...
    <div class="input_fields_wrap {{$name}}">
        ...
        <div class="input-group">
            <input type='text' title="{{$form['label']}}"
                   {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} {{isset($validation) && isset($validation['max'])?"maxlength=".$validation['max']:""}} class='form-control {{$name}} first_value'
    ...
    @push('bottom')
        <script>
            $(document).ready(function () {
                var max_fields_{{$name}}    = "{{ isset($form['max_fields']) ? $form['max_fields'] : '' }}";
                ...
                $(add_button_{{$name}}).click(function (e) { //on add input button click
                    ...
                    if (count_{{$name}} < max_fields_{{$name}} ) { //max input box allowed
                        ...
                        $(wrapper_{{$name}}).append('<div><input class="form-control" {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} {{isset($validation) && isset($validation['max'])?"maxlength=".$validation['max']:""}} type="text" name="{{$name}}[]"/><a href="#" class="remove_field {{$name}}"><i class="fa fa-minus"></a></div>'); //add input box
                    }
                });
                ...
                function Load() {
                    ...
                    for (i = 1; i < val.length; i++) {
                        $(wrapper_{{$name}}).append(' <div > <input class="form-control" {{$required}} {{$readonly}} {!!$placeholder!!} {{$disabled}} {{isset($validation) && isset($validation['max'])?"maxlength=".$validation['max']:""}}  type="text" name="{{$name}}[]" value="' + val[i] + '"/><a href="#" class="remove_field {{$name}}"><i class="fa fa-minus"></a></div>'); //add input box
                    }
                }
